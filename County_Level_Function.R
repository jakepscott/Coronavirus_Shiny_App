...
county_graph <- function(Data,
  ...
  Data <- left_join(Data,Increasing_or_Decreasing, by=c("Date","State","County")) 
  ...
  Data <- Data %>% 
    mutate(Label_New_Cases=paste("There were ",round(New_Cases,0), " new cases reported in ",
                                 County, ", ", State, " on ", as.character(month(Date, label = T,abbr = F)),
           ...
           Label_Cases=paste("There were ", round(Cases,0), " cumulative cases in ",
                             County, ", ", State, " as of ", as.character(month(Date, label = T,abbr = F)),
                             " ", day(Date), ", ", year(Date),sep=""),
...
#county_graph(US_Data, rollmean = 10, state = "Alabama", county = "Autauga", measure = "Cases",per_million = F)
