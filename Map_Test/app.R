...
server <- function(input, output) {
    ...
    US_Data <- reactive({
        withProgress(message = "Getting your requested data", {
                         incProgress(amount = 1/4, message = "Cleaning raw data")
                         Data <- 
                             US_Data_Raw %>% 
                         ...
                         incProgress(amount = 3/4, message = "Calculating statistics", 
                                     detail = "This will take a little bit...")
                         Data <- Data %>% 
                             arrange(County,Date) %>% 
                             group_by(fips) %>% 
